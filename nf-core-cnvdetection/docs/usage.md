# nf-core/cnvdetection: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/cnvdetection/usage](https://nf-co.re/cnvdetection/usage)
...
```bash
nextflow run nf-core/cnvdetection --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```bash
nextflow pull nf-core/cnvdetection
```
...
First, go to the [nf-core/cnvdetection releases page](https://github.com/nf-core/cnvdetection/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/cnvdetection`](https://hub.docker.com/r/nfcore/cnvdetection/)
* `singularity`
